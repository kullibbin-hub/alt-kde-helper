...
class MainWindow(QMainWindow):
    ...
    def create_maintenance_page(self):
        ...
        cards.append(SimpleActionCard(
            "Очистка кэша",
            "Очистка кэша в профиле пользователя, Flatpak приложений и приложений\nиз репозитория и epm, аккуратно чистит /tmp (stmpclean), удаляет старые журналы\n(оставляет только 100 МБ), чистит кэш пакетов (apt-get clean).\nAutoremove - НЕ ИСПОЛЬЗУЕТСЯ!!!\nПоказывает полный отчет о проделанной работе.",
            "95_clean_cache_action.sh"
        ))
